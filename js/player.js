...
function createPlayer(game, position, color, texture_names, gravity, bounce)
{
    var 	main_player,	// Object all sprites
		...
		p_left_leg_sprite_anim = p_lLeg_sprite.animations.add("stay",  [0,1,1], frame_rate/2, true),
		p_right_leg_sprite_anim = p_rLeg_sprite.animations.add("stay",  [0,1,1], frame_rate/2, true);
		...
		main_player = {
			...
			{	//Head relative position
				this.head.sprite.position.x = this.body.sprite.position.x - this.head.sprite.width/3>>0;
				this.head.sprite.position.y = this.body.sprite.position.y;

				if( this.dirrection == 1) {//If left dirrect
					this.leg_left.sprite.position.x = this.body.sprite.position.x - this.leg_left.sprite.width + 10;
					this.leg_left.sprite.position.y = this.body.sprite.position.y +
													this.body.sprite.height - this.leg_left.sprite.height;

					this.leg_right.sprite.position.y = this.leg_left.sprite.position.y;
					this.leg_right.sprite.position.x = this.body.sprite.position.x +
													   this.body.sprite.width - 
													   this.leg_right.sprite.width + 7;
				}
				else { //If right dirrect
					this.leg_left.sprite.position.x = this.body.sprite.position.x + 10;
					this.leg_left.sprite.position.y = this.body.sprite.position.y +
			...
			debugInfoToConsole: function()
			{
				console.log("head.x = %d, head.y = %d, \
							 ...
							 this.head.sprite.position.x, this.head.sprite.position.y,
							 this.leg_left.sprite.position.x, this.leg_left.sprite.position.y,
							 this.leg_right.sprite.position.x, this.leg_right.sprite.position.y);
			}
		};
