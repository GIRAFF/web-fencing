...
function createPlayer(game, position, color, texture_names, gravity, bounce)
{
    var 	main_player,	// Object all sprites
	    ...
	    p_head_sprite_anim = p_head_sprite.animations.add("stay",
														  [0,1,2,3,2,1],
														  frame_rate, 
														  true),

		p_body_sprite_anim = p_body_sprite.animations.add("stay",
														 [0,1,2,3,2,1],
														 frame_rate,
														 true),

		p_left_leg_sprite_anim = p_lLeg_sprite.animations.add("stay", 
															 [0,1,1],
															 frame_rate/2, 
															 true),

		p_right_leg_sprite_anim = p_rLeg_sprite.animations.add("stay",
															   [0,1,1],
															   frame_rate/2,
															   true);
		...
		main_player = {
			...
			{	//Head relative position
				this.head.sprite.position.x = this.body.sprite.position.x 
											- this.head.sprite.width/3>>0;
				...
				if(this.dirrection == 1) {//If left dirrect
				   ...
				   this.body.sprite.position.x - this.leg_left.sprite.width + 10;

				   this.leg_left.sprite.position.y =
				   this.body.sprite.position.y + this.body.sprite.height -
				   ...
				   this.body.sprite.position.x + this.body.sprite.width - 
				   this.leg_right.sprite.width + 7;
					...
					this.leg_right.sprite.position.x = 
					//Прости, иначе не влезало
					this.body.sprite.position.x + this.leg_right.sprite.width-8;
				}
			...
			jump: function()
			{
			    if(this.on_ground && game.time.now > this.jump_timer) {
			        this.body.sprite.body.velocity.y = this.power_jump;
					this.jump_timer = game.time.now + 1500;
				}
			...
			left: function()
			{
				//this.body.sprite.body.velocity.x = - this.velocity_horizontal;
				this.body.sprite.body.position.x -= 4;
			},
